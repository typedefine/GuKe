//
//  ZJNChangePatientBodyInfoViewController.m
//  GuKe
//
...
@interface ZJNChangePatientBodyInfoViewController ()<UITableViewDelegate,UITableViewDataSource,ZJNChoosePCellDelegate,ChoseViewDelegate,MWPhotoBrowserDelegate,UINavigationControllerDelegate,ZYQAssetPickerControllerDelegate,UIImagePickerControllerDelegate,ZJNPatientBodyInfoDelegate,TZImagePickerControllerDelegate>
{
...
@implementation ZJNChangePatientBodyInfoViewController
...
- (void)viewDidLoad {
    ...
    for (int a = 0; a < self.infoModel.tzxx.count; a ++ ) {
        NSString *imgPATH = [NSString stringWithFormat:@"%@%@",imgPath,self.infoModel.tzxx[a][@"path"]];
        [imageArr addObject:imgPATH];
    }
...
-(void)nextButtonClick:(UIButton *)button{
    ...
    NSString *imgFile;
    if (imageArr.count>0) {
        imgFile = [imageArr componentsJoinedByString:@","];
    }
    imgFile =[imgFile stringByReplacingOccurrencesOfString:imgPath withString:@""];
    NSString *urlStr = [NSString stringWithFormat:@"%@%@",requestUrl,updatetzxx];
    NSArray *keysArr;
    NSArray *valuesArr;
    if (imgFile){
     keysArr = @[@"sessionId",@"hospnumId",@"temperature",@"pulse",@"breathe",@"pressure",@"tzxx"];
     valuesArr = @[self.infoModel.sessionId,self.infoModel.hospnumId,self.infoModel.temperature,self.infoModel.pulse,self.infoModel.breathe,self.infoModel.pressure,imgFile];
    }else{
        keysArr = @[@"sessionId",@"hospnumId",@"temperature",@"pulse",@"breathe",@"pressure"];
         valuesArr = @[self.infoModel.sessionId,self.infoModel.hospnumId,self.infoModel.temperature,self.infoModel.pulse,self.infoModel.breathe,self.infoModel.pressure];
    }
    ...
    [ZJNRequestManager postWithUrlString:urlStr parameters:dic success:^(id data) {
        ...
        if ([retcode isEqualToString:@"0000"]) {
            if (self.refreshBodyInfoBlock) {
                self.refreshBodyInfoBlock();
            }
...
-(void)initInfoStr{
    NSArray *arr = [self.infoModel.pressure componentsSeparatedByString:@"/"];
    if (![self.infoModel.pressure isEqualToString:@""]) {
        highBP = arr[0];
        lowBP = arr[1];
    }else{
        highBP = @"";

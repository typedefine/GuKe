...
{
    return [[xsqbanner alloc]initWithtFrame:frame ImageArr:imageNameArray AndImageClickBlock:clickBlock];
}
...
{
    return [[xsqbanner alloc]initWithtFrame:frame ViewArr:customViewArr AndImageClickBlock:clickBlock];
}
...
#pragma mark 设置pagecontrol 位置和颜色
-(void)MakePageFramRight{
    
    self.pageVC.frame =CGRectMake( self.frame.size.width -  20 * _imageArr.count,self.frame.size.height-30,20 * _imageArr.count, 30);
...
-(void)addImageToScrollView
{
    ...
    for (NSString *name in imgMArr) {
        ...
        if(imgView.image ==nil)
        {
            ...
            [imgView sd_setImageWithURL:[NSURL URLWithString:name] placeholderImage:[UIImage imageNamed:@"future"]];
        }
        //设置图片的坐标
...
-(void)setPlaceholderName:(NSString *)placeholderName
{
    ...
    for (UIView *view in self.direct.subviews) {
        if([view isKindOfClass:[UIImageView class]])
        {
            UIImageView *subImageView=(UIImageView *)view;
            [subImageView sd_cancelCurrentImageLoad ];
            [subImageView sd_setImageWithURL:subImageView.sd_imageURL placeholderImage:[UIImage imageNamed:placeholderName]];
        }
    }
}

...
// tag
static const NSInteger CSToastTag       = 2000;
...
{
    UIView *toast = [self viewWithTag:CSToastTag];
    if(!toast)
    {
        toast = [self viewForMessage:message title:nil image:nil];
        toast.tag = CSToastTag;
...
- (CGPoint)centerPointForPosition:(id)point withToast:(UIView *)toast {
    if([point isKindOfClass:[NSString class]]) {
        ...
        if([point caseInsensitiveCompare:@"top"] == NSOrderedSame) {
            ...
            return CGPointMake(self.bounds.size.width/2, (self.bounds.size.height/2 + 7*toast.frame.size.height));

...
//    [self makeZhuJian];
    ...
    judge = 0;
    titleArr = @[@[@"手术日期",@"手术名称",@"手术入路",@"手术医生",@"第一助手",@"第二助手"]];
    placeHolderArr = @[@"请选择手术时间",@"请输入手术名称",@"请输入手术入路",@"请输入手术医生",@"请输入第一助手",@"请输入第二助手"];
...
- (id<MWPhoto>)photoBrowser:(MWPhotoBrowser *)photoBrowser photoAtIndex:(NSUInteger)index{
    MWPhoto *photot;
    if (signIndexPath.section == 3) {
         NSString * photimageStr  =[NSString stringWithFormat:@"%@",self.requestModel.qx[index]];
        if ([photimageStr containsString:imgPath]) {
         ...
         NSString * photimageStr  =[NSString stringWithFormat:@"%@",self.requestModel.imagex[index]];
        if ([photimageStr containsString:imgPath]) {
        }else{
            photimageStr = [NSString stringWithFormat:@"%@%@",imgPath,photimageStr];
        }
         ...
         NSString * photimageStr  =[NSString stringWithFormat:@"%@",self.requestModel.imaget[index]];
        if ([photimageStr containsString:imgPath]) {
        }else{
            photimageStr = [NSString stringWithFormat:@"%@%@",imgPath,photimageStr];
        }

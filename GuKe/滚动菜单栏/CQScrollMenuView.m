...
- (void)setTitleArray:(NSArray *)titleArray{
    ...
    [_titleArray enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        if (_lastView) {
            menuButton.frame = CGRectMake(_lastView.right + 15, 0, 30, self.height);
        }else{
            menuButton.frame = CGRectMake(10, 0, 30, self.height);
        }
//        [menuButton setBackgroundColor:RandomColor];
        menuButton.tag = 100 + idx;
        [menuButton.titleLabel setFont:[UIFont systemFontOfSize:16]];
        [menuButton setTitleColor:[UIColor blackColor] forState:UIControlStateNormal];
        [menuButton setTitleColor:_selectBtnColor?_selectBtnColor:[UIColor redColor] forState:UIControlStateSelected];
        ...
        if (idx == 0) {
            ...
            _lineView.backgroundColor = _lineColor?_lineColor : [UIColor redColor];
        }
        
        _lastView = menuButton;
    }];
    
    self.contentSize = CGSizeMake(CGRectGetMaxX(_lastView.frame) + 10, CGRectGetHeight(self.frame));

...
@interface WYYShipinViewController ()<UITableViewDelegate,UITableViewDataSource,UITextFieldDelegate>{
...
@implementation WYYShipinViewController
...
- (void)makeAddTableview{
    if (IS_IPGONE_X) {
        self.orderTable = [[UITableView alloc]initWithFrame:CGRectMake(0, search.bottom, ScreenWidth, ScreenHeight-41 - 88 - search.height) style:UITableViewStylePlain];
    }else{
        self.orderTable = [[UITableView alloc]initWithFrame:CGRectMake(0, search.bottom, ScreenWidth, ScreenHeight-41 - 64- search.height) style:UITableViewStylePlain];
    }
...
- (void)getDataFromService{
    ...
    NSString *urlString = [NSString stringWithFormat:@"%@%@",requestUrl,pubvideomorelist];
    NSArray *keysArray = @[@"sessionId",@"videoTypeId",@"videoName"];
    NSArray *valueArray = @[sessionIding,self.model.videoTypeId,searchField.text];

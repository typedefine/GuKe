...
//    }
    ...
    NSArray *keysArray = @[@"sessionid",@"doctorName",@"gender",@"birth",@"email",@"hospitalId",@"clinic",@"deptId",@"titleId",@"num",@"content",@"portrait",@"specialty"];
    NSArray *valueArray = @[sessionIding,doctorName,gender,birth,email,hospitalId,clinic,deptId,titleId,num,content,portrait,specialty];
    NSDictionary *dic = [NSDictionary dictionaryWithObjects:valueArray forKeys:keysArray];
    [self showHudInView:self.view hint:nil];
    [ZJNRequestManager postWithUrlString:urlString parameters:dic success:^(id data) {
        NSLog(@"%@",data);

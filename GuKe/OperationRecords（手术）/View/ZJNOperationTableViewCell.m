...
-(id)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier{
    ...
    if (self) {
        ...
        [self.contentView addSubview:self.surgeryTypeLabel];
        ...
        [self.surgeryTypeLabel mas_makeConstraints:^(MASConstraintMaker *make) {
            make.left.right.equalTo(self.secondLabel);
            make.height.equalTo(@35);
            make.top.equalTo(self.firstLabel.mas_bottom).offset(6);
...
-(UILabel *)surgeryTypeLabel{
    if (!_surgeryTypeLabel) {
        _surgeryTypeLabel = [[UILabel alloc]init];
        _surgeryTypeLabel.font = SetFont(14);
        _surgeryTypeLabel.numberOfLines = 2;
        _surgeryTypeLabel.textColor = SetColor(0x1a1a1a);
    }
    return _surgeryTypeLabel;
}
...
-(void)setModel:(OperationInfoModel *)model{
    _model = model;
    self.model.surgeryTypeName =[NSString changeNullString:self.model.surgeryTypeName];
    ...
    self.surgeryTypeLabel.text =[NSString stringWithFormat:@"手术类别：%@",self.model.surgeryTypeName];
    [Utile setUILabel:self.surgeryTypeLabel data:@"手术类别：" setData:self.model.surgeryTypeName color:SetColor(0x666666) font:14 underLine:NO];

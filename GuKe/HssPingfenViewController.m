...
@interface HssPingfenViewController ()<WKUIDelegate,WKNavigationDelegate>{
    WKWebView *harrWek;
}
...
@implementation HssPingfenViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    self.title = @"HSS评分(膝关节)";
    ...
    if ([self.valStr isEqualToString:@"(null)"]||[self.valStr isEqualToString:@""]||[self.valStr isEqualToString:@"<null>"]||(self.valStr == nil)) {
        urlString = [NSString stringWithFormat:@"%@/app/hss/list.json",requestUrl];
    }else{
        urlString = [NSString stringWithFormat:@"%@/app/hss/list.json?val=%@&pfuid=%@",requestUrl,self.valStr,self.pfuid];
    }
    ...
    if ([str rangeOfString:@"harris/val.json?val"].location != NSNotFound) {
        ...
        [ZJNRequestManager postWithUrlString:urlString parameters:nil success:^(id data) {
            ...
            if ([retcode isEqualToString:@"0"]) {
                NSDictionary *valuesDic = data[@"data"];
                if (self.returnValueHsBlock) {
                    self.returnValueHsBlock(valuesDic);
                }

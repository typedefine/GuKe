//
//  WYYChoseGroupNumberViewController.m
//  GuKe
//
...
@interface WYYChoseGroupNumberViewController ()<UITableViewDelegate,UITableViewDataSource,UITextFieldDelegate>{
...
@implementation WYYChoseGroupNumberViewController

- (instancetype)initWithStyle:(selectTheContectStyle)style{
    self = [super init];
    if (self) {
        self.style = style;
    }
...
- (void)onOKNeedView{
    ...
    if (self.style == 0) {
        if (weakself.backgroupnumber) {
            self.backgroupnumber(numberArr);
        }
    }else{
        if (weakself.backaddnumber) {
            self.backaddnumber(numberArr);
        }
    }

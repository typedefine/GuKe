...
- (NSArray<GroupOperationItemModel *> *)items
{
    if (!_items) {
        ...
        NSMutableArray<GroupOperationItemModel *> *targetList = [NSMutableArray arrayWithCapacity:titles.count];
        ...
        if (self.isStudio) {
            if (!self.isManager) {
                [targetList removeObjectAtIndex:1];
                [targetList removeLastObject];
            }
        }else{
            targetList[0].name = @"工作组介绍";
            [targetList removeObjectAtIndex:1];
            [targetList removeLastObject];
        }

...
- (void)viewDidLoad {
    ...
    friendsModel  = [_db getFriendInfoModel:self.conversation.conversationId];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([object isKindOfClass:[NSString class]]) {
        ...
        if(self.conversation.type == EMConversationTypeChat){
            if(!sendCell.model.isSender){
        
                [sendCell.avatarView sd_setImageWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@%@",imgPath,friendsModel.portrait]] placeholderImage:model.avatarImage];
            }
...
{
    EMMessage *message = [EaseSDKHelper getCmdMessage:action to:self.conversation.conversationId messageType:[self _messageTypeFromConversationType] messageExt:ext cmdParams:nil];
...
- (void)sendTextMessage:(NSString *)text
{
    ...
    [ext setValue:[NSString stringWithFormat:@"%@%@",imgPath,ChatImgUrl] forKey:@"userPic"];
    [ext setValue:[NSString stringWithFormat:@"%@",ChatUserName] forKey:@"userName"];
    [self sendTextMessage:text withExt:ext];
}
                         ...
                         andAddress:(NSString *)address
{
    ...
    EMMessage *message = [EaseSDKHelper getLocationMessageWithLatitude:latitude longitude:longitude address:address to:self.conversation.conversationId messageType:[self _messageTypeFromConversationType] messageExt:dic];
    [self _sendMessage:message isNeedUploadFile:NO];
}
...
- (void)sendImageMessageWithData:(NSData *)imageData
{
    ...
    EMMessage *message = [EaseSDKHelper getImageMessageWithImageData:imageData to:self.conversation.conversationId messageType:[self _messageTypeFromConversationType] messageExt:dic];
...
- (void)sendImageMessage:(UIImage *)image
{
    ...
    NSDictionary *dic = @{@"userPic":[NSString stringWithFormat:@"%@%@",imgPath,ChatImgUrl],@"userName":[NSString stringWithFormat:@"%@",ChatUserName]};
    EMMessage *message = [EaseSDKHelper getImageMessageWithImage:image to:self.conversation.conversationId messageType:[self _messageTypeFromConversationType] messageExt:dic];
    [self _sendMessage:message isNeedUploadFile:YES];
}
                             ...
                             duration:(NSInteger)duration
{
    ...
    EMMessage *message = [EaseSDKHelper getVoiceMessageWithLocalPath:localPath duration:duration to:self.conversation.conversationId messageType:[self _messageTypeFromConversationType] messageExt:dic];
    [self _sendMessage:message isNeedUploadFile:YES];
}
...
- (void)sendVideoMessageWithURL:(NSURL *)url
{
    ...
    NSDictionary *dic = @{@"userPic":[NSString stringWithFormat:@"%@%@",imgPath,ChatImgUrl],@"userName":[NSString stringWithFormat:@"%@",ChatUserName]};
    EMMessage *message = [EaseSDKHelper getVideoMessageWithURL:url to:self.conversation.conversationId messageType:[self _messageTypeFromConversationType] messageExt:dic];
    [self _sendMessage:message isNeedUploadFile:YES];
}

//
//  HuiYiXiangQingViewController.m
//  GuKe
//
...
@interface HuiYiXiangQingViewController ()<WKNavigationDelegate,WKUIDelegate>{
...
@implementation HuiYiXiangQingViewController
...
- (void)shareWebPageToPlatformType:(UMSocialPlatformType)platformType
{
    ...
    //创建网页内容对象
    NSString* thumbURL = [NSString stringWithFormat:@"%@%@",imgPath,self.iconImagePatn];
    UMShareWebpageObject *shareObject = [UMShareWebpageObject shareObjectWithTitle:self.titleStr descr:self.content thumImage:thumbURL];
    //设置网页地址
    shareObject.webpageUrl = [NSString stringWithFormat:@"%@/app/meeting/fxorganization_show.json?uid=%@",requestUrl,self.huiyiid];
...
- (void)makeAddView{
    ...
    NSString *urlString = [NSString stringWithFormat:@"%@/app/meeting/organization_show.json?uid=%@",requestUrl,self.huiyiid];
    NSURL *url = [NSURL URLWithString:urlString];
    NSURLRequest *request = [NSURLRequest requestWithURL:url];
    [detailWeb loadRequest:request];
    [self.view addSubview:detailWeb];
    
    //之前添加在网页的代理方法里边，会出现进入页面要等一会儿才能加载的问题
